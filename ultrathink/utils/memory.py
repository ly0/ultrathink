...
This module provides functions to collect and load AGENTS.md memory files
from user-level and project-level locations. Memory files contain persistent
instructions that are injected into the system prompt.
"""
...
def collect_all_memory_files(force_include_external: bool = False) -> List[MemoryFile]:
    ...
    Args:
        force_include_external: If True, allow includes outside cwd
    ...
    # Global/user-level memories live in home and ~/.ultrathink.
    user_memory_paths = [
        Path.home() / ".ultrathink" / MEMORY_FILE_NAME,
...
def build_memory_instructions() -> str:
    ...
    Returns:
        Formatted string with all memory file contents, or empty string if none found
    """
